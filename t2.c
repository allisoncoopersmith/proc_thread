...
int search (int arrSz, int numThreads, int target1, int* numbers)
{

   int arraySize = arrSz; //user specified
   //int n; //number of threads, user specified
   target = target1; //user specified
   if((target>arraySize-1)|| target < 0){
	printf("No no no, no no no no no no no, no no no no. This is out of bounds franny boi\n");
	return -1;
   }

   //int numbers[arraySize]; moved to outside
...
// this is printing the array
 printf("-------------------------------------\n");*/
   ...
   pthread_t *thread;
   n = numThreads; //number of threads we want

	if(n>arraySize){
		printf("Error! Number of threads > arraysize. Impossible ya dummy boi.\n");
		return -1;
	}
